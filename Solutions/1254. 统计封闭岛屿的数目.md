...
class Solution:
    ...
    def dfs(self, grid, i, j):
        ...
        if i<0 or j<0 or i>=m or j>=n:
            return False
        if grid[i][j] !=0 :  #--------只会在dfs的过程中return，结束非必要的遍历
            return True
        grid[i][j] = 1
        
        ans1 = self.dfs(grid, i+1, j)
        ans2 = self.dfs(grid, i-1, j)
        ans3 = self.dfs(grid, i, j-1)
        ans4 = self.dfs(grid, i, j+1)
        if ans1 and ans2 and ans3 and ans4: #所有方向的遍历都合法，才属于孤立岛屿
            return True
        else:
            return False
 ```
