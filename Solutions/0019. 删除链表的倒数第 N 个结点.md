...
- 链表存储的方式，嵌套式存储
- 关于指针浅拷贝的理解：最终结果打印
cur : ListNode{val: 3, next: ListNode{val: 5, next: None}}
dummpy：ListNode{val: 0, next: ListNode{val: 1, next: ListNode{val: 2, next: ListNode{val: 3, next: ListNode{val: 5, next: None}}}}}
head：ListNode{val: 1, next: ListNode{val: 2, next: ListNode{val: 3, next: ListNode{val: 5, next: None}}}
...
class Solution:
    def removeNthFromEnd(self, head: ListNode, n: int) -> ListNode:
        ...
        # cur和dummy都是指针，python中指针等号赋值都是浅拷贝
        cur = dummy
        for i in range(length - n):
            cur = cur.next
        cur.next = cur.next.next
        return dummy.next #扔掉假的头部，返回真正原始的链表头
